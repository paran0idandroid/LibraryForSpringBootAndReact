...
//    private RebackMapper rebackMapper;
    ...
    public ResultDTO updateBorrowInfo(BookBorrowDTO bookBorrowDTO) {
        int booId = bookBorrowDTO.getBookId();
        ...
        //1 查找本地是否存在这个书籍
        BookDTO bookDTO = bookMapper.findById(booId);
        if(null == bookDTO){
        ...
        if(!StringUtils.isEmpty(localBookBorrowDTO.getTmp1())){
          ...
          if(tmp1Count == borrowCount){
              return new ResultDTO(HttpCode.SUCCESS.getCode(), "您已还完全部书籍，无需归还");
          }
        }
//        if(tmp1Count == borrowCount){
//            return new ResultDTO(HttpCode.SUCCESS.getCode(), "您已还完全部书籍，无需归还");
//        }
    ...
    private ResultDTO doReback(int rebackCount, int borrowCount, BookBorrowDTO localBookBorrowDTO, BookDTO bookDTO){
        ...
        if(rebackCount > borrowCount){
            ...
            //1.还书数量小于借书数量，更新还书数量（tmp1），更新book剩余数量
        ...
        return new ResultDTO(HttpCode.SUCCESS.getCode(), "还书成功，还有" +
                (borrowCount - rebackCount) +
                "本书尚未归还,请在：" + localBookBorrowDTO.getEndDate() + "之前归还剩余书籍");
    }
}
